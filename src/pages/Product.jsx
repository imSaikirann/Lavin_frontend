            ...
            {dataStatus === 'loading' ? (
                ...
                <div className="flex gap-12 sm:gap-12 flex-col sm:flex-row">
                    <div className="flex-1 flex flex-col-reverse gap-3 sm:flex-row">
                                                ...
                                                <div key={index} className="flex-shrink-0">
                                                    {page.images.map((image, imgIndex) => (
                                                        <div key={imgIndex} className="text-center">
                                                            <img
                                                                src={`${s3Url}/${image}`}
                                                                alt={`Internal page ${index + 1} image ${imgIndex + 1}`}
                                                                className="w-[100px] h-auto cursor-pointer"
                                                                onClick={() => setSelectedImageIndex(imgIndex)} // Set selected image index on click
                                                            />
                                                        </div>
                                                    ))}
                                                </div>
                        ...
                        {/* Display selected product image (initially the first cover image) */}
                        <div className="w-full sm:w-[80%]">
                            {selectedProduct && selectedProduct.images.length > 0 && (
                                <img
                                    src={`${s3Url}/${selectedProduct.images[selectedImageIndex]}`} // Display based on selected index
                                ...
                                {selectedProduct.variants.length > 0 ? (
                                    <div className="mt-3 flex flex-wrap gap-2">
                                        {selectedProduct.variants.map((variant, index) => (
                                            <button
                                                key={index}
                                                onClick={() => handleVariantChange(index)} // Change the variant on click
                                                className={`px-4 py-2 border-2 rounded-lg transition-colors duration-200 
                            ${selectedVariantIndex === index ? 'bg-black text-white border-transparent' : 'bg-white text-black border-gray-300'}
